...
export default function CreatePostPage() {
  ...
  function surFichiersChoisis(e) {
    ...
    e.target.value = '';
    setEditionOuverte(true); // ouvre directement l'ecran d'edition, comme sur TikTok
  }
  ...
  function rendreMedia(tailleGrande) {
    if (!activeMedia) return null;
    const hauteur = tailleGrande ? '100%' : undefined;
    return (
      <>
        ...
        <div style={{ padding: 16 }}>
          ...
          {mediaItems.length > 0 && (
            <div
              ...
              style={{ position: 'relative', borderRadius: 14, overflow: 'hidden', marginBottom: 12, background: '#0C0A06', aspectRatio: (activeMedia.ratio || 1) + ' / 1', cursor: 'pointer' }}
            >
              {activeMedia.kind === 'video' ? (
                <video src={activeMedia.url} style={{ width: '100%', height: '100%', objectFit: 'cover', transform: activeMedia.mode === 'cover' ? 'translate(' + activeMedia.offsetX + 'px,' + activeMedia.offsetY + 'px) scale(' + activeMedia.zoom + ')' : 'none', filter: styleFiltreActif() }} muted loop autoPlay playsInline />
              ) : (
                <img src={activeMedia.url} alt="media" style={{ width: '100%', height: '100%', objectFit: 'cover', transform: activeMedia.mode === 'cover' ? 'translate(' + activeMedia.offsetX + 'px,' + activeMedia.offsetY + 'px) scale(' + activeMedia.zoom + ')' : 'none', filter: styleFiltreActif() }} />
              )}
