    ...
    <AdminShell>
      <div style={{ background: '#0C0A06', backgroundImage: BOGOLAN_DARK, padding: '44px 14px 14px', borderRadius: '0 0 24px 24px', marginBottom: 14 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <button onClick={function() { navigate('/parish-admin/branches/nouveau'); }} style={{ background: 'linear-gradient(135deg,#C8A84B,#8B6020)', border: 'none', borderRadius: 20, padding: '7px 14px', fontSize: 9, color: VERT, fontWeight: 700, fontFamily: 'Georgia,serif', cursor: 'pointer' }}>+ Creer</button>
        </div>
      </div>
          ...
          return (
            <div key={g._id} style={{ background: 'white', borderRadius: 16, padding: 14, border: '1px solid rgba(0,0,0,0.06)', opacity: g.isActive ? 1 : 0.6 }}>
              <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 10 }}>
                <div style={{ width: 44, height: 44, borderRadius: 12, background: g.photoUrl ? 'transparent' : (g.type === 'prive' ? 'linear-gradient(135deg,#1e2d14,#0a140a)' : 'rgba(200,168,75,0.1)'), display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, border: '1.5px solid ' + (g.type === 'prive' ? 'rgba(200,168,75,0.3)' : 'rgba(200,168,75,0.2)'), overflow: 'hidden' }}>
                  {g.photoUrl
