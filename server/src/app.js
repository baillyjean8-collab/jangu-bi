...
if (!origin || allowed.includes(origin)) {
 ...
 // ── 4 & 5. JSON body parser ───────────────────────────────────────────────
  ...
  app.use((req, res, next) => {
    if (req.path.includes('/avatar') || req.path.includes('/posts') || req.path.includes('/stories') || req.path.includes('/parishes')) return next(); // ces routes acceptent des photos en base64, plus volumineuses
